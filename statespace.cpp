...
long double f1(long double count, long double epsylon){
  long double c1 = 1;
  long double c2 = 1;
  c2 = epsylon/((epsylon+c2)*(epsylon+c2));
  c1 = c1/(exp((c2)*(epsylon/2))-c1);
  long double ans = c1*(exp(-c2*(count)));
  // cerr << ans << endl;
  return ans;
}
set <pair<int,int>> st;
map <vector<pair<int,set<pair<int,int>>>>,int> mp;
int cmax = 1;
int pmax = 196;
int cno = 2;
int sno = 2;
...
void stateSpace::exploreStateSpace() {
  ...
  cerr << endl;
  long double epsy = 1;
  long double weight = 2*init[init.size()-2];
  epsy = epsy/(epsy + weight);
  cerr << epsy << endl;
  A.reset(N.getState());
  ...
  while (!toBeExplore.empty()) {
    ...
    for (size_t t = 0; t < N.tr; t++){
      for (let b : N.Transition[t].bindingList) {
        ...
        if (N.IsEnabled(t, b)) {
          N.fire(t, b, 0.0);
          int SE = A.synchroniseWith(t, N.getState(), b); //fire it in the LHA
          if (SE > -1) {
            // if the LHA responds(there is a transition in the LHA)
            // cerr << "transition:" << *it << endl;
            vector <pair<int,set<pair<int,int>>>> vect;
            vector<int> marking = N.getState().getVector(); // new marking
            bool hi = true;
            for (int i=2;i<2+((cmax+2)*pmax)*(cno-1);i++){
              if (marking[i]>1 || marking[i]<0){
                hi = false;
                break;
              }
            }
            ...
            count0 = marking[1];
            temp.insert({0,count0});
            vect.push_back({0,temp});
            temp.clear();
            int count = 0;
            int count2= 0;
            int clientno = 1;
            for (int i = 2;i<2+((cmax+2)*pmax)*(cno-1);i++){
              ...
              if (count == ((cmax+1)*pmax)){
                int counter = 1;
                set <pair<int,int>> tempo;
                for (auto j : temp){
                  tempo.insert({counter,j.second});
                  counter++;
                }
                channelno++;
                clientno =  1;
                vect.push_back({channelno,tempo});
                temp.clear();
                count = 0;
              }
            }
            set <pair <int,int >> tempset = {{0,0}};
            for (int i=2+((cmax+2)*pmax)*(cno-1);i<2+((cmax+2)*pmax)*(cno-1)+sno;i++){
              vect.push_back({marking[i],tempset});
            }
            if (mp[vect]==0 && hi){
              // for (auto i : marking) cerr << i << " ";
              // cerr << endl;
              mp[vect]++;
              nbTrans++;
              long double f = f1(marking[0]-1,epsy);
              if (marking[0]<=marking[marking.size()-1] && marking[0]!=1){              
                ...
                // Param[0] is the weight of the transition
                marking.push_back(A.CurrentLocation);
                auto its = S.find(&marking); // checking if the marking already exists
                if (its == S.end()) {
                  // if the marking doesn't exist
                  toBeExplore.push(marking);
                  add_state(marking); //add the marking to the state space 
                  // we can make a change here on whether we should add the marking or not, depending on the value of the functions f0 and f1
                  // then we can create 3 more vectors containing elements of AlmostLoose0, AlmostLoose1, and A'.
                  // this way we will be able to restrict the state space.
                  // now this is only to create a finite state space, I still need to figure out how does COSMOS generate a markov chain.. 
                  //and take into account the transition probabilities
                }
              }
            }
          }
  ...
  // for (auto i : *findstate){
  //   for(auto j : i) cerr << j << " ";
  //   cerr << endl;
  // }
  // cerr << "AlmostLoose1\n";
  // for (auto i : AlmostLoose1){
  //   for (auto j : i) cerr << j << " ";
  //   cerr << endl;
  // }
  // cerr << "Aprime\n";
