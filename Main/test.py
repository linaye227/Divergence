...
def poly(n,l):
    k = 1
    ans = 0
    for i in range(len(l)-1,-1,-1): #Finds the value of the polynomial respresented in the list L with x = n
        ans+=k*l[i]
        k*=n
    return ans
theta = 0.01
...
while (True):
    ...
    gamma = math.exp((epsy/(epsy+1))**2) # this function calculates the maximum amount of clients in the system corresponding to the value of theta
    if (gamma==1): break #we aim to set an epsylon such that this maximum number of clients is as small as possible
    x = -(math.log((math.sqrt(gamma)-1)*theta))/math.log(gamma) #the details of this allgorithm are explained in the developer manual also uploaded on git
    maxim = x + minimum
    minimum+=1
    ...
    if (l[0]=="C"):
        ...
        for j in range(1,len(l)):
            if (j-1!=0) : 
                ...
                out.write('<attribute name="variableDeclaration"><attribute name="name">x</attribute><attribute name="type">client</attribute></attribute></attribute>')
